...
1. Read `/tasks/done/TASK-XXXX.md`.
...
1. Do not fix code.
2. Do not change the task except moving it to `/tasks/rejected` on rejection.
...
2. Leave the task in `/tasks/done`.
3. Provide evidence for each pin test.
...
2. Move the task from `/tasks/done` to `/tasks/rejected`.
...
In `controlled` mode, a rejected report stops the workflow until the user decides. In `auto` mode, the orchestrator may return the task to `developer` only if the fix is safe and does not require requirement clarification.
