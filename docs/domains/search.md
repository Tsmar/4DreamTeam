...
Search storage participates in the shared schema registry. When search tables are created or validated, the tool records the search schema version and hash in `tool_schema_versions`, so agents can inspect drift with `4dt-db schema status` instead of reading SQLite internals directly.
...
- `packages/db/src/fourdt_db/cli.py` exposes schema status for recorded tool schema versions.
...
- Store rebuildable search chunks/manifests in the shared database rather than `.4dt/search` JSONL files.
